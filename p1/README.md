...
| Machine verification of Theorem B in full (sandwich C(k,e0) <= k c(k) <= (k+1) C(k,e0)) against Mathlib | `cd lean_mathlib && lake exe cache get && lake build` (Lean v4.33.1, Mathlib v4.33.1; downloads the Mathlib cache, ~7 GB) | `CollatzB.theoremB`, axioms within `[propext, Classical.choice, Quot.sound]`, no sorry |
