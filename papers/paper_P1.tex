...
=O(N^{0.950})$, the same entropy exponent. The counting theorem itself, and the core
arithmetic lemmas behind the robustness and rate theorems (the climbing
towers $-1\bmod 2^m$, the affine identity, the collapse and persistence
lemmas, and the rotation lemma), are machine-verified in Lean~4 from the
standard axioms. We
...
\item[(iii)] \emph{Fine structure} (Theorems~C--E): the climbing condition
  ...
  compression.
\item[(iv)] \emph{Machine verification}: Theorem~A itself is verified
  in Lean~4 in an abstract certificate-system formulation (as an
  injection from climbing classes into deep certificates), and so are
  the core arithmetic lemmas behind Theorems~A$'$ and B --- the
  climbing towers $-1\bmod 2^m$, the affine identity with its remainder
  bound, both halves of the adaptive collapse lemma, the persistence
  step, and the rotation lemma --- all from the standard axioms alone
...
\item \texttt{Counting.lean}: \emph{Theorem A itself}, in an abstract
  ...
  Quot.sound]}.
\end{itemize}
Of the main results, Theorem~A is thus machine-verified in full (modulo
the trivial reading of an injection as a cardinality bound), and
Theorems~A$'$ and B have their arithmetic cores machine-verified; the
remaining steps of A$'$ and B, and Theorems~C--G, are verified by
conventional proof plus the independent computations described above.
...
\texttt{Rotation.lean} & \texttt{20f91338f3d623a496ce1d0a6b1ff775}\\[-2pt]
 & \texttt{75f14921756e17dc13ab48bd5863b97a}\\
\texttt{Counting.lean} & \texttt{09619f3b7862934a14ef7fb9ed9d4646}\\[-2pt]
