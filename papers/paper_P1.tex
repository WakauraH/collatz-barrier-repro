...
=O(N^{0.950})$, the same entropy exponent. The core arithmetic lemma
(classes $-1 \bmod 2^m$ climb for $m-1$
steps with unit $2$-adic valuations) is machine-verified in Lean~4. We
...
\item[(iii)] \emph{Fine structure} (Theorems~C--E): the climbing condition
  ...
  compression.
\item[(iv)] \emph{Machine verification}: the core arithmetic lemma --- the
  classes $-1 \bmod 2^m$ climb monotonically for $m-1$ steps with unit
  $2$-adic valuations --- is verified in Lean~4 from the standard axioms
  alone (Section~\ref{sec:artifacts}).
...
never descend by (a) and step $j_1$ descends by (b).
\end{proof}
...
claim; the third is the Chinese remainder theorem.
\end{proof}
...
way $n\le 2(3/2)^{j_1}j_1^{13.3}/c$, which is smaller than $2^m<n$ for
...
\subsection{Machine verification of the core lemma}
Lemma~\ref{lem:climb} is verified in Lean~4 as four theorems
(\texttt{climb}, \texttt{climb\_mod}, \texttt{climb\_ge}, \texttt{barrier})
in the artifact \texttt{Barrier.lean}, compiled with core Lean only (no
external libraries), toolchain pinned to
\texttt{leanprover/lean4:v4.32.2}. \texttt{\#print axioms} reports exactly
\texttt{[propext, Quot.sound]}, the standard kernel axioms, and no
\texttt{sorry}. We emphasize that the machine verification covers this core
lemma only; the counting and rate theorems are verified by conventional
