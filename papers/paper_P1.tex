...
counting theorem, the sandwich, and the core lemmas are machine-verified
in Lean~4.
\end{abstract}
...
\item[(iv)] \emph{Machine verification}: Theorems~A and B are verified
  ...
  injection from climbing classes into deep certificates), B in full
  against Mathlib --- and so are the core arithmetic lemmas behind
...
\item \texttt{TheoremB.lean} (separate Lake project, Lean
  ...
  \emph{Theorem B in full}. Words are \texttt{List.Vector Bool k}, the
...
\end{itemize}
Of the main results, Theorems~A and B are thus machine-verified in full
(A modulo the trivial reading of an injection as a cardinality bound;
B against Mathlib), and Theorem~A$'$ has its arithmetic core
machine-verified; the remaining steps of A$'$, and Theorems~C--G, are
verified by conventional proof plus the independent computations
described above. The three core files use no library at all and can be
checked in seconds; the Theorem~B project requires a Mathlib cache.
