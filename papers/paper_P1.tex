...
bounds on its complexity. Every correct certificate system contains, for
every $k$, at least $c(k)$ certificates of dyadic depth exceeding $k$,
where $c(k)$ counts the climbing classes mod $2^k$; the growth rate of
$c(k)$ is exactly the binary entropy $H(\log_3 2)=0.9500$, with
non-asymptotic two-sided constants. The bound is robust under disjunctions
of congruence classes of arbitrary moduli and adaptive stopping times, and
the same collapse yields an unconditional bound $N^{H(\log_3 2)}$ on the
number of potential counterexamples up to $N$. The certificate frontier is
driven by the Sturmian word of slope $\log_3 2$ and is not regular. The
counting theorem and the core lemmas are machine-verified in Lean~4.
\end{abstract}
...
\noindent\textbf{Word count:} approximately 6{,}700 words.
