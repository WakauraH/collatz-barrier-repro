...
decay and flagged the exponential improvement as open. We prove five
structural theorems: an exact one-dimensionalization of the Fourier
...
$2^K \bmod 3^m$ on full periods. With an audited weighted-$\ell^1$
bookkeeping these yield a complete conditional proof of
$\delta_m\le\mathrm{poly}(m)\,2^{-0.0504\,m}$, whose single unproven
input is isolated as a finite-orbit quantitative form of Furstenberg
$\times 2\times 3$ rigidity. For fixed $m$ and finitely many frequencies
that input is decidable: we certify it, with all numerical error
accounted, on the full extended window at $m=64$, $128$, $256$. Certified
computations to $m=1200$ support the sharp-rate conjecture
...
\noindent\textbf{Word count:} approximately 5{,}200 words.
...
single isolated arithmetic statement, and certify that statement's inputs
--- with all numerical error accounted --- on the full frequency window
at fixed levels $m\le 256$. Our results are of four
...
respectively, and the band-rate inputs of the conditional theorem hold on
$W_m$ with positive slack at rates $0.0350$, $0.0406$, $0.0457$ increasing
toward $0.0504$ (Section~\ref{sec:instances}). Statements about
...
$L/(2\alpha h-c)+1$; summing over $h$,
\[
\sum_{\text{entries}}\nu_2(v_{\mathrm{entry}})\;\le\;
\frac{L}{2\alpha}\bigl(\ln h_{\max}+O(1)\bigr)+h_{\max},
\qquad h_{\max}\le\log_2(\varepsilon 3^d),
\]
deterministically. This is exactly the shape of the logarithmic loss:
the implied bound on black steps, $(L/4\alpha)(\ln h_{\max}+O(1))$,
exceeds $L$ once $d\gtrsim 250$, so it guarantees no white points
...
is precisely the missing ingredient. (A saturated population allowed by
the lemma realizes the $\sim 0.6\log_2 d$ per-entry average
numerically.)
\end{remark}
...
state that the band-rate inputs of Theorem~\ref{thm:conditional} hold on
$W_m$ at the given $(\mu,r(\mu))$ with the stated minimal slack (zero
safety margin: all numerical error is inside $U_m$).
\end{theorem}

\begin{remark}[scope]\label{rem:instscope}
(i) The certified rate of the constraint system increases toward the
conditional rate $0.0504$ ($0.0350\to0.0406\to0.0457$, one doubling at a
time), with the binding constraints stabilizing in the shoulder band
$u\in[-13,-9]$ --- the same band the asymptotic analysis identifies as
the precision zone; we regard this as structural corroboration of the
inputs of Theorem~\ref{thm:conditional}. (ii) Under window localization
...
the step law is truncated at $b\le 40$ and the omitted mass is added as
$(m/2)\sum_{b>40}(b-1)2^{-b}$ (union bound, $|f|\le 1$); pruned dynamic
program states are added back at their true scale (future factors are at
most $1$); each $|f|$ receives $+10^{-11}$ and the total a factor
$1+10^{-8}$, both exceeding the floating-point error by more than two
orders of magnitude. The certified numbers reproduce the unaccounted
floating-point ones to within $10^{-4}$ in rate.
\end{remark}
