...
\begin{abstract}
Let $\delta_m$ be the maximal nontrivial Fourier coefficient of the
Syracuse random variable modulo $3^m$ --- the quantity whose decay drives
Tao's almost-all result for the Collatz map; Tao proved superpolynomial
decay and flagged exponential decay as open. We prove five structural
theorems: an exact one-dimensionalization of the Fourier
problem onto the exponent lattice; a first-passage renormalization
identity; a weighted contraction lemma with optimal rate exactly
$2^{-I_0}$, $I_0=(1-H(\log_3 2))/\log_3 2$; the exact Kesten tail
exponent $1$; and an exact geometric law for the $2$-adic valuations of
$2^K \bmod 3^m$ on full periods. With a window-audited weighted-$\ell^1$
bookkeeping these yield a proof architecture for
$\delta_m\le\mathrm{poly}(m)\,2^{-rm}$, $r>0.045$, with two inputs: a
local equidistribution hypothesis (a finite-orbit quantitative form of
Furstenberg $\times 2\times 3$ rigidity) and band-rate estimates decidable
on finite frequency sets, which we certify with error accounted on the
full window at $m=64$, $128$, $256$.
Certified computations to $m=1200$ support the sharp-rate conjecture
$\delta_m=\Theta(2^{-I_0m}m^{-c})$.
\end{abstract}

\noindent\textbf{Keywords:} $3x+1$ problem; Syracuse random variable;
Fourier decay; $3$-adic equidistribution; Kesten chain; Furstenberg
$\times 2\times 3$ rigidity; experimental mathematics.

\noindent\textbf{2020 Mathematics Subject Classification:} Primary 11B83;
Secondary 11K16, 37P10, 60H25.

\noindent\textbf{Word count:} approximately 5{,}000 words.
...
prove the structural theorems that reduce the upper-bound problem to one
isolated arithmetic hypothesis plus finitely decidable band-rate inputs,
and certify the latter --- with all numerical error accounted --- on the
full frequency window at fixed levels $m\le 256$. Our results are of four
kinds, and we keep the four statuses --- \emph{theorem},
\emph{computational fact} (exact or certified computation),
\emph{conditional proposition}, and \emph{conjecture} --- explicitly labeled
throughout.
...
Cauchy-type boundary, $\Prob(Y>t)\sim C/t$ with $C=1/\log(27/16)$
...
\subsection*{Conditional proposition} Assembling (i)--(v) with a
capped weighted-$\ell^1$ bookkeeping on the frequency window (audited
exactly on all $399$ one-step inequalities between consecutive computed
levels, with zero violations) and sharp product bounds along Tao's
two-dimensional update process, we obtain a proof architecture for
\[
\delta_m\;\le\;\mathrm{poly}(m)\cdot 2^{-r\,m},\qquad r>0.045,
\]
--- the qualitative form of the improvement flagged in \cite{Tao2022} ---
with two unproven inputs. One is a set of band-rate estimates (BR) for
the product bounds, which are decidable on any finite set of frequencies
and which we certify on the full extended window for $m\le 256$
(Section~\ref{sec:instances}); the other (Hypothesis~\ref{hyp:clocal})
is a local equidistribution statement: entries of the orbit
...
consistent with $I_0=0.07932$; a boundary-layer structure theory
(Section~\ref{sec:mech}); and certified instances on the full extended
frequency window at $m=64$, $128$, $256$: unconditionally
$\sup_{k\in W_m}|g_m(2^k)|\le 2^{-4.97},\ 2^{-8.4},\ 2^{-15.0}$
respectively, and the band-rate inputs (BR) hold on $W_m$ outside the
critical strip with positive slack at rates $0.0350$, $0.0406$, $0.0457$
--- a rate that plateaus at $0.0457$ from $m=192$ on, the product-bound
rates near the window edge extrapolating to $\approx 0.049$
(Section~\ref{sec:instances}). Statements about
$\delta_m$ itself at fixed $m$ additionally require the extremal frequency
to lie in $W_m$, which is verified exhaustively for $m\le 15$.
...
\item[(b)] (first-passage decomposition) With
  ...
  exceeds $k$.
\item[(c)] (kernel bound) $|F(d,r)|\le 2^{-r}\,\delta_{d-1}$ for $d\ge 2$.
\end{enumerate}
\end{theorem}
...
\eqref{eq:rec} with weight $2^{-\lambda u}$ (so that $\lambda^*>0$ below
corresponds to a weight decaying above the line, matching the convention
$\mu=-\lambda^*$ of \eqref{eq:rmu}):
\[
...
\end{equation}
the contraction rate of the capped weighted-$\ell^1$ \emph{window}
functional
\[
N_{\mu}(m)=\sum_{k\in W_m}\min\bigl(2^{\mu u},\,1\bigr)\,|g_m(k)|,
\]
$W_m$ the extended window of Section~\ref{sec:instances}; one checks
$\max_{\mu}r(\mu)=I_0$ at $\mu=-\lambda^*$, and smaller $|\mu|$ trades
rate for weaker demands on the frequencies below the window. The index set
must be a window: summed over all of $\mathbb Z$ the functional diverges
by periodicity of $g_m$, and on a centered fundamental domain of the
exponent circle it does not contract (the two levels have different
periods), so the global formulation carries no content beyond the window
one; frequencies entering the window from below are the band-rate inputs
(BR) of Section~\ref{sec:cond}.

\begin{compfact}[audited bookkeeping]\label{cf:audit}
The one-step inequality $N_{\mu}(m)\le 2^{-r(\mu)}N_{\mu}(m-1)$ of the
window functional with $\mu=-0.16$, $r(\mu)=0.0504$, was audited against
the exact lattice data for all $399$ pairs of consecutive levels
$2\le m\le 400$: zero violations, with no injection terms required within
the window. The bookkeeping at the window boundary is exactly the
constraint system (BR) used in the certified instances of
...
\]
The multiplier $\log(3\cdot 2^{-a})=\log 3-a\log 2$ takes its values
in the shifted lattice $\log 3-(\log 2)\mathbb{Z}$, whose generated closed
subgroup is dense because $\log 3/\log 2$ is irrational; its law is
therefore \emph{non-arithmetic}, the Goldie conditions hold, and
\[
\Prob(Y>t)\sim\frac{C}{t},\qquad
C=\frac{\E[2^{-a}]}{\E[3\cdot 2^{-a}\log(3\cdot 2^{-a})]}
=\frac{1}{\log(27/16)}=1.91114\ldots,
\]
so $\E Y=\infty$ and the tail exponent is exactly $1$.
\end{theorem}
...
$3=2^2-1$. The moment condition of implicit renewal theory
\cite{Kesten1973,Goldie1991} is immediate
($\E[(3\cdot2^{-a})\log^+(3\cdot2^{-a})]<\infty$), and in the
non-arithmetic case Goldie's theorem gives $\Prob(Y>t)\sim Ct^{-1}$ with
$C=\E[B]/\E[A\log A]$ for $A=3\cdot 2^{-a}$, $B=2^{-a}$; here
$\E[B]=1/3$ and $\E[A\log A]=\log 3-\tfrac43\log 2=\tfrac13\log(27/16)$.
\end{proof}

This places Syracuse at the \emph{boundary} Cauchy-type case: power
corrections in $1/m$ are to be expected, which is
...
Exhaustive verification for $m\le 12$ over the full period and
$\sigma\le 6$ found zero mismatches of Lemma~\ref{lem:val}, and the
observed frequencies at $m=12$, $0.5000,\,0.2500,\,0.1250,\,0.0625$ (for
both $\varepsilon=1/2$ and $\varepsilon=1/10$), match the geometric law. We also note a caveat used below: value chains generated
...
\item[(b)] Rescaled lattice computations for $m\le 1200$: the extremal
  ...
  maximum lies in $[0.0790,\,0.0800]$ bits/level, straddling
  $I_0=0.07932$ within the measurement resolution, consistent with a
  polynomial correction.
\item[(c)] Only one side of (b) is accessible to the theory: weighted
...
the collision representation (with $Z=X_1+X_2-X_3-X_4$ for four
independent copies)
$S_4'(m)=3^m\,\Prob(Z\equiv 0 \bmod 3^m)-3^{m-1}\,\Prob(Z\equiv 0 \bmod 3^{m-1})$,
the subtracted term removing the frequencies divisible by $3$; exact
computation
...
\section{Toward unconditional exponential decay}
\label{sec:cond}
...
Let $w_1,w_2$ be the odd seeds of two chains of depth $\ge h$ (i.e.\
chains reaching $\nu_2\ge h$, so $|w_i|\le\varepsilon 3^d/2^h$) in the
same row, at positions $e_1<e_2$. Then
\[
e_2-e_1\ \ge\ \log_2\!\Bigl(\frac{2^h}{\varepsilon}-1\Bigr)
\ \ge\ h+\log_2(1/\varepsilon)-1 .
\]
\end{lemma}

\begin{proof}
Put $\Delta=e_2-e_1\ge 1$. Then $2^{\Delta}w_1\equiv w_2\pmod{3^d}$ and
$2^{\Delta}w_1\ne w_2$ (one side is even, the other odd), so
$|2^{\Delta}w_1|+|w_2|\ge 3^d$; with $|w_i|\le\varepsilon 3^d/2^h$ this
gives $(2^{\Delta}+1)\varepsilon/2^h\ge 1$, which is the claim.
\end{proof}
...
($|2v|\le 3^d/2$ precludes reduction); Lemma~\ref{lem:exhaust} combines the
...
windows of length $O(m)=O(\log q)$, $q=3^d$. Measured behavior: in the
...
\begin{remark}[what Lemma~\ref{lem:pair} alone gives, and why it is not enough]
\label{rem:pairalone}
The pair-spacing lemma does yield an unconditional statement: along a
monotone segment of length $L$ the walk enters each chain at most once,
an entry of valuation $\ge h$ lies in a chain of depth $\ge h$, and
Lemma~\ref{lem:pair} bounds the number of such chains by
$L/(h+\log_2(1/\varepsilon)-1)+1$; summing over $h$,
\[
\sum_{\text{entries}}\nu_2(v_{\mathrm{entry}})\;\le\;
L\bigl(\ln h_{\max}+O(1)\bigr)+h_{\max},
\qquad h_{\max}\le\log_2(\varepsilon 3^d),
\]
deterministically. This is exactly the shape of the logarithmic loss:
the implied bound on black steps, $(L/2)(\ln h_{\max}+O(1))$,
exceeds $L$ already for small $d$, so it guarantees no white points
asymptotically, and a per-entry average of order $\log d$ follows only
when the number of entries is itself of order $L$ --- which is again an
equidistribution input. Thus Lemma~\ref{lem:pair} cannot replace
Hypothesis~\ref{hyp:clocal}: it controls the \emph{population} of deep
chains but not their occupation by the walk, and the geometric depth law
is precisely the missing ingredient.
\end{remark}
...
each fixed $m$ on any finite set of frequencies, which is what
Section~\ref{sec:instances} exploits.

\subsection{The conditional theorem}

\begin{proposition}[conditional exponential decay]\label{thm:conditional}
Assume Hypothesis~\ref{hyp:clocal}, and assume the band-rate estimates
\begin{itemize}
\item[(BR)] for some $\mu<0$ and all $m$, the sharpened product bounds
  along Tao's update process (all-$b$ dynamic program) satisfy
  $|g_m(2^k)|\le 2^{-a(\delta)m}$ with $a(\delta)\ge r(\mu)+|\mu|\delta$,
  $\delta=|k-m\alpha|/m$, for every frequency outside the critical strip
  $-8\le k-m\alpha\le 2$ of the window, and $a\ge 0.25$ at the seam.
\end{itemize}
Then $\delta_m\le C\,\mathrm{poly}(m)\,2^{-r(\mu)m}$. Numerically, (BR)
is certified on the extended window with $\mu=-0.14$ ($r=0.0457$) at
$m=192$ and $256$ (Theorem~\ref{thm:instances}); the value $\mu=-0.16$
($r=0.0504$) obtained from rates sampled at $m=240$ is not stable under
finer sampling in $m$, and the certified product-bound rates near the
window edge extrapolate to $\approx 0.049$ (Remark~\ref{rem:instscope}),
so we claim only $r>0.045$.
\end{proposition}
...
$a(\delta)$ computed by the all-$b$ product DP --- this is where (BR)
enters as an input rather than a consequence: the rates sampled at
$m=240$ and $320$ (ridge $0.060$--$0.078\le I_0$ as a consistency check,
wrap side $0.094$--$0.17$, below-line $0.0633/0.0810/0.108$ at
$\delta=0.05/0.15/0.30$, seam $\approx 0.33$) close the constraint
$r(\mu)+|\mu|\delta\le a(\delta)$ at $\mu=-0.16$, but the certified
full-window computations of Section~\ref{sec:instances} close it only at
$\mu=-0.14$ for $m\le 256$, with the shoulder band binding. Given (BR),
the capped window functional's one-step contraction at rate $r(\mu)$ is
the audited inequality of Computational Fact~\ref{cf:audit}, the seam and
below-window terms are controlled by (BR), and summing the one-step
inequalities gives the claim.
\end{proof}

\begin{remark}
The division of labor is explicit: every step of the architecture is
either proved (Theorems~\ref{thm:onedim}--\ref{thm:cstat},
Lemmas~\ref{lem:r1a}--\ref{lem:pair}), audited on the window
(Computational Fact~\ref{cf:audit}), or an input --- Hypothesis
\ref{hyp:clocal} and the band-rate estimates (BR). Theorem~\ref{thm:cstat}
proves the full-period form of the former; (BR) is certified level by
level in Section~\ref{sec:instances} but not proved uniformly in $m$. We
do not currently see how to prove the windowed form of the hypothesis,
and we flag it as the precise obstruction --- in our view a more tractable
target than the Collatz conjecture itself, but of the $\times 2\times 3$
family.
\end{remark}

\section{Certified instances on the window}\label{sec:instances}

For each fixed $m$ and each fixed frequency, Hypothesis~\ref{hyp:clocal}
is not needed: Tao's product inequality (7.5) is a theorem, and its
right-hand side $B_m(k)$ at the frequency $2^k$ is a finite computation.
We stress at the outset what such computations can and cannot certify:
they bound $|g_m(2^k)|$ for the frequencies actually scanned, and no
finite scan reaches the $2\cdot 3^{m-1}$ frequencies of a level $m\ge
16$; statements about $\delta_m$ itself therefore rest on the window
localization of the extremal frequency (exhaustively verified for
$m\le 15$, Computational Fact~\ref{cf:numerics}(a)).

\begin{theorem}[finite certification on the window]\label{thm:instances}
Fix $m$ and a finite set $W$ of exponents. For each $k\in W$ the product
bound $B_m(k)$ is computable with every approximation accounted on the
upper side (truncation of the step law, pruning of the dynamic program,
and floating-point rounding), yielding numbers $U_m(k)\ge |g_m(2^k)|$.
On the extended window $W_m=[-80,\,\floor{1.66m}+20]$:
\[
\begin{array}{lllll}
m=64: & 207\ \text{freq.}, & \sup_{W_m}|g_m|\le 2^{-4.97}, &
\mu=-0.10,\ r=0.0350, & \text{slack } +0.0078;\\
m=128: & 313\ \text{freq.}, & \sup_{W_m}|g_m|\le 2^{-8.4}, &
\mu=-0.12,\ r=0.0406, & \text{slack } +0.021;\\
m=256: & 525\ \text{freq.}, & \sup_{W_m}|g_m|\le 2^{-15.0}, &
\mu=-0.14,\ r=0.0457, & \text{slack } +0.010.
\end{array}
\]
Here the window supremum bound is unconditional, and the last two columns
state that the band-rate estimates (BR) hold on $W_m$ outside the critical
strip $-8\le u\le 2$ (which the row-sum identity handles) at the given
$(\mu,r(\mu))$ with the stated minimal slack (zero safety margin: all
budgeted numerical error is inside $U_m$).
\end{theorem}

\begin{remark}[scope]\label{rem:instscope}
(i) Finer sampling in $m$ shows that the certified constraint rate has
already plateaued: $\mu=-0.10,-0.11,-0.12,-0.13,-0.14,-0.14$ at
$m=64,96,128,160,192,256$ ($r=0.0350,0.0379,0.0406,0.0432,0.0457,0.0457$),
with the binding constraints in the shoulder band $u\in[-13,-9]$. The
certified window rates fit $a_{\min}(m)=0.0491+0.31\log_2 m/m$ to within
$10^{-4}$ over $m=64$--$256$ (shoulder band: $0.048+0.40\log_2 m/m$), so the product-bound rates near the window edge
extrapolate to $\approx 0.048$--$0.049$, below $r(-0.16)=0.0504$; the
$(\log_2 m)/m$ correction is the finite-size trace of the logarithmic loss
that Hypothesis~\ref{hyp:clocal} is meant to remove. Whether the sampled
rate $0.0504$ is attainable by this bookkeeping is therefore open, and we
do not claim it. (ii) Under window localization
of the extremal frequency, $\delta_m=\sup_{W_m}|g_m|$ and the exact window
computation gives the much smaller true value
(e.g.\ $\log_2\delta_{256}\approx-30.7$); the product bound is the
\emph{certifiable} envelope, not the truth. (iii) The error accounting:
the step law is truncated at $b\le B(m)$, $B=41,65,112$ for
$m=64,128,256$, chosen so that the omitted mass
$(m/2)\sum_{b>B}(b-1)2^{-b}$ (union bound, $|f|\le 1$) is below
$2^{-0.35m-8}$; pruned dynamic program states are added back at their true
scale (future factors are at most $1$), with an $m$-adaptive threshold
$2^{-0.4m-40}$; each $|f|$ receives $+10^{-11}$ and the total a factor
$1+10^{-8}$. The two allowances are \emph{budgeted}, not
interval-certified: they exceed by more than two orders of magnitude the
error implied by $\le 1$\,ulp accuracy of the library sine and cosine on
reduced arguments and by the operation counts of the dynamic program. The
certified numbers reproduce the unaccounted floating-point ones to within
$10^{-4}$ in rate.
\end{remark}
...
\[
(\mathrm{L}\alpha)\ \ \sup_{k\ge m\alpha+C_0}|g_m(k)|\le 2^{-I_0m+o(m)}\ \text{for some constant } C_0
\qquad\quad
...
Since \cite{Tao2022} appeared, manuscripts not yet peer-reviewed have claimed the
upgrade of its logarithmic-density conclusion to natural density
(\cite[Remark~1.16]{Tao2022}) through sum-conditioned versions of the
superpolynomial decay \cite{Allikvere2026}. Whatever their eventual
status, they use only superpolynomial decay of $\delta_m$; the
exponential question studied here is independent of them.
...
J.~Allikvere,
\emph{Almost all Collatz orbits attain almost bounded values in natural
density}, v2, Zenodo (July 2026), \url{https://doi.org/10.5281/zenodo.21499244}
...
was \emph{not} met (the measured range is $[0.0790,0.0800]$). After
seeing the data we adopted a one-sided criterion (rate $\ge I_0-\epsilon$,
approaching $I_0$), on the structural ground that certified functionals
bound the rate from one side only; the data satisfy that criterion, but
this is a post-hoc replacement and we record it as such. We keep this log because, in our experience, the two
