...
- **P1** — *Certificate complexity of uniform descent for the 3x+1
  ...
  power-saving count N^{0.950} of potential counterexamples; the core
  arithmetic lemma machine-verified in Lean 4.
- **P2** — *The 3-adic mixing rate of the Syracuse random variable*
  ...
  periods; unconditional certified bounds at m = 64, 128, 256; certified
  numerics to m = 1200 and the sharp-rate conjecture
...
p1/       one self-contained artifact per computational claim of P1
          (Lean proof + pinned toolchain, certificate JSON, independent
          verifier, exact-arithmetic scripts) — see p1/README.md
p2/       same for P2 (exponent-lattice computations, audits, band-rate
          DPs, certification drivers + logs) — see p2/README.md
```
...
Expected: four theorems, axioms `[propext, Quot.sound]` only.
